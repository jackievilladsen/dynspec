...
def add_band(ma_big,t,f,ma_band,t_band,f_band):
    ...
    # convert ma_big to complex numbers
    ma_big = ma_big.astype(complex)
...
class Dynspec:
    ...
    def load_dynspec(self,params):
        ...
        if os.path.exists(filename):
            ...
            self.spec['rr'] = make_ma(load(filename+'/rr.npy'))[i:,:]
            print 'RCP rms:', self.get_rms('rr')*1000, 'mJy'
            
            print 'loading LL from', filename
            self.spec['ll'] = make_ma(load(filename+'/ll.npy'))[i:,:]
            print 'LCP rms:', self.get_rms('ll')*1000, 'mJy'
            
            print 'loading RL from', filename
            self.spec['rl'] = make_ma(load(filename+'/rl.npy'))[i:,:]
            print 'RL rms:', self.get_rms('rl')*1000, 'mJy'
            
            print 'loading LR from', filename
            self.spec['lr'] = make_ma(load(filename+'/lr.npy'))[i:,:]
            print 'LR rms:', self.get_rms('lr')*1000, 'mJy'
    ...
    def get_rms(self,pol='ll'):
        ...
        if self.spec[pol].shape[1] > 1:
            rms = ma.median(ma.std(self.spec[pol],0))
            try:
                return rms[0]
            except:
                return rms
        else:
            return ma.std(self.spec[pol])
    ...
    def plot_dynspec(self,plot_params={}):
        ...
        if scale=='log':
            imshow(log10(self.spec[pol]).T,aspect=ar,vmin=log10(smin),vmax=log10(smax),origin='lower',cmap='seismic',interpolation='nearest')
            ...
            imshow(self.spec[pol].T,aspect=ar,vmin=smin,vmax=smax,origin='lower',cmap='seismic')
        ...
        tlist = self.get_tlist()  # get time since beginning of observation (in units of integration time)
        t_minutes = tlist * self.dt()/60.  # convert to time in minutes
        xmax = max(t_minutes)
        xtick_lbls = arange(0,xmax+dx,dx)
        tick_labels,tick_locs = make_tick_labels(xtick_lbls,t_minutes)
        xticks(tick_locs, tick_labels)
        xlabel('Time in minutes since '+ self.t0() +' UTC')
        ...
        '''
        imin = closest_ind(t_bin/60.,min(tC_min))
        imax = closest_ind(t_bin/60.,max(tC_min))
        di = imax-imin
        jmin = closest_ind(f_bin/1.e9,min(fC_GHz))
        jmax = closest_ind(f_bin/1.e9,max(fC_GHz))
        dj = jmax-jmin
        '''
...
filelist = ['UVCet_3L.dynspec','UVCet_3S.dynspec']
...
ar = 7.0 # aspect ratio setting for large dynspec (binned)
...
'''
dynspecL = Dynspec(params={'filename':'UVCet_3L.dynspec','uniform':True})
dynspecS = Dynspec(params={'filename':'UVCet_3S.dynspec','df':2.e6})
...
title('UV Ceti RCP Dynamic Spectrum - 07/04/2015')
savefig('dynspecRCP.pdf',bbox_inches='tight')

ds_bin.plot_dynspec(plot_params={'dx':30,'pol':'ll','smin':0.014,'smax':0.139})
...
# load and plot P band data
# for now just plot time series so I can identify which pol is which
dynspecP = Dynspec(params={'filename':'UVCet_3P.dynspec','uniform':True})
nt = 8   # already binned to 15 sec integrations
nf = 128
tseriesP = dynspecP.bin_dynspec(nt,nf)
figure()
t = arange(len(tseriesP.spec['rr']))*nt/4.
plot(t,tseriesP.spec['rr'].real[:,1])
hold(True)
plot(t,tseriesP.spec['ll'].real[:,1])
plot(t,tseriesP.spec['rl'].imag[:,1])
plot(t,tseriesP.spec['lr'].imag[:,1])
legend(['re(XX)','re(YY)','im(XY)','im(YX)'])
savefig('tseriesP.pdf',bbox_inches='tight')

figure()
tseriesI = (tseriesP.spec['rr']+tseriesP.spec['ll']).real/2.
tseriesV = (tseriesP.spec['rl']-tseriesP.spec['lr']).imag/2.
plot(t,tseriesI[:,1])
hold(True)
plot(t,tseriesV[:,1])
legend(['Stokes I','Stokes V'])
savefig('tseriesP_stokes.pdf',bbox_inches='tight')

figure()
nt = 8 # 4 min integrations
nf = 16 # 
ar = 2.0 * nf/nt
dsPbin = dynspecP.bin_dynspec(nt,nf)
dsPbin.spec['I']=(dsPbin.spec['rr']+dsPbin.spec['ll']).real/2
dsPbin.plot_dynspec(plot_params={'dx':30,'df':0.1,'pol':'I','rmspol':'I'})
title('UV Ceti Stokes I Dynamic Spectrum - 07/04/2015')
savefig('dynspecP_I.pdf',bbox_inches='tight')
print dsPbin.get_rms('I')
dsPbin.spec['V']=(dsPbin.spec['rl']-dsPbin.spec['lr']).imag/2
print dsPbin.get_rms('V')
dsPbin.plot_dynspec(plot_params={'dx':30,'df':0.1,'pol':'V','rmspol':'V'})
title('UV Ceti Stokes V Dynamic Spectrum - 07/04/2015')
savefig('dynspecP_V.pdf',bbox_inches='tight')
