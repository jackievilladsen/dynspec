...
for srcname in srclist:
    ind = find([s == srcname for s in src])
    plot(ind*t_int,imag(flux[ind])-0.5,'.')
legend(srclist,loc='upper left')
title(survey)
xlabel('Time in seconds (zero point is arbitrary)')
ylabel('Imaginary component')
ymin = min(imag(flux)-0.5)*1.1
axis([0,len(times)*t_int,ymin,max(real(flux))*1.1])
ylabel('Flux (Jy) at 1 pc')
smin = 1.5*median(flux_err) # minimum flux value for histogram
#axhline(S10,color='r')
axhline(0.,color='k')
axhline(-0.5,color='k')
axhline(smin,color='r')
axhline(smin-0.5,color='r')
