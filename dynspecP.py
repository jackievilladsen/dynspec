...
# split(vis='/data/jrv/15A-416/UVCet/3/P/UVCet_3P.star.ms',outputvis='UVCet_6P.star.ms',spw='0:700~1000',scan='0~30',datacolumn='data')
...
try:
    fields = get_fields(srcms,vishead)
except:
...
try:
    ...
    pixel_size,imsize = im_params(ms1spw)
if imsize > 10000:
...
threshold='100.0mJy'
clean(vis=srcms,imagename='test2',scan=clean_scans,spw=clean_spws,nterms=1,imsize=imsize,cell=pixel_size,niter=1000,threshold=threshold,cyclefactor=10,cyclespeedup=10,interactive=True)

clean(vis=srcms,imagename='test',field=srcname,scan=clean_scans,spw=clean_spws,nterms=1,imsize=imsize,cell=pixel_size,niter=1000,threshold=threshold,cyclefactor=10,cyclespeedup=10,interactive=interactive)
