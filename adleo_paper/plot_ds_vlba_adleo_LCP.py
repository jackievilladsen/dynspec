'''
plot_ds_vlba_adleo.py - Load ADLeo or UVCet multi-band dynamic spectrum for a given epoch, bin to specified resolution, and plot to file
 - different from uvcet version in that it does not plot dynamic spectrum of VLBA data.
'''
...
for epoch in epochlist:
    ...
    ds_bin.spec['v'] = abs(real(ds_bin.spec['v']))
    dsP_bin.spec['v'] = abs(real(dsP_bin.spec['v']))
    ...
    cb.remove()
    figfile = savedir + src + '_' + epoch + '_dynspec_LCP.pdf'
